package com.example.lab1_pizza
import android.content.Intent
import androidx.appcompat.app.AppCompatActivity
import android.os.Bundle;
import android.view.View
import android.widget.*
import com.example.lab1_pizza.Models.Pizza
...
class MainActivity : AppCompatActivity() {
 ...
 lateinit var total: TextView
    ...
    fun ingredientsClicked(view: View) {
        if(mozzarellaCheck.isChecked)
            pizza.mozzarellaCheesePrice = 1.0
        else
            pizza.mozzarellaCheesePrice = 0.0

        if(parmesanCheck.isChecked)
            pizza.parmezanCheesePrice = 1.0
        else
            pizza.parmezanCheesePrice = 0.0

        if(mascarponeCheck.isChecked)
            pizza.mascarponeCheesePrice = 1.0
        else
            pizza.mascarponeCheesePrice = 0.0

        if(hamCheck.isChecked)
            pizza.royalHamPrice = 2.0
        else
            pizza.royalHamPrice = 0.0

        if(bavarianCheck.isChecked)
            pizza.bavarianSalamiPrice = 1.0
        else
            pizza.bavarianSalamiPrice = 0.0

        if(tomatoesCheck.isChecked)
            pizza.freshTomatoesPrice = 1.0
        else
            pizza.freshTomatoesPrice = 0.0

        if(baconCheck.isChecked)
            pizza.baconPrice = 1.0
        else
            pizza.baconPrice = 0.0

        if(champCheck.isChecked)
            pizza.champignonsPrice = 1.0
        else
            pizza.champignonsPrice = 0.0

        if(pepperoniCheck.isChecked)
            pizza.pickledPepperoniPrice = 1.0
        else
            pizza.pickledPepperoniPrice = 0.0

        if(olivesCheck.isChecked)
            pizza.greenOlivesPrice = 1.0
        else
            pizza.greenOlivesPrice = 0.0
        calcTotal()
        total.text="Total: " + pizza.totalPrice

    }
    fun calcTotal(){
        pizza.totalPrice = pizza.sizePrice + pizza.greenOlivesPrice + pizza.pickledPepperoniPrice +
